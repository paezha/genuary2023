...
The prompt for Day 4 is pretty open ended: intersections. I thought of intersecting geometries using package [`sf`](https://r-spatial.github.io/sf/) but instead I will intersect colors using regular geometries.
...
For the final image, the regular polygons are "intersected" with the horizontal bands. The alternation of colors creates the illusion of vanishing intersections.

Randomly select a color palette from packages [{MetBrewer}]() or [{MexBrewer}](https://paezha.github.io/MexBrewer/):
```{r}
set.seed(seed)
...
# Display image
knitr::include_graphics(glue("outputs/transchromation-{seed}.png"))
```
